...
;Input:
;Output: R0
;Reg Modifications: R0 will be 0 if key wasnt pressed, 1 if it was
...
GetCount
;store value of word variable Count in the address in R0
;Calls:
;Input:
;Output: R0
;Reg Modifications: R0 will be value of word variable Count
            PUSH  	{R1}
            LDR   	R1,=Count
            LDR   	R1,[R1,#0]
			MOVS  	R0,R1
...
TimeStart	PROC    {R0-R14},{}
;initialize Count and start timer
			PUSH 	{R0-R1}
...
;Change value of LED
;Calls: 
;Input: bit 0 set for green, bit 1 set for red
;Output:
;Reg Modifications:
            PUSH    {R0-R2}
            ;addresses for green
            ...
            ;shift value of green to carry bit
            LSRS    R0,R0,#1
            ;check if it's set
            BCS     GRNSET
            ;turn off green
...
REDCHK
            ...
            LDR     R1,=FGPIOE_BASE
            LDR     R2,=LED_RED_MASK
            ;shift value of red to carry bit
            LSRS    R0,R0,#1 
            ;check if set
            BCS     REDSET
            ;else turn red off
