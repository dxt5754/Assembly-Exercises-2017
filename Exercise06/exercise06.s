...
NEWSTRING   
			LDR     R0,=Prompt				;load promt address into R0
			BL      PutStringSB				;print the prompt
			LDR 	R0,=STRINGIN			;load input string address into R0
			LDR 	R1,=MAX_STRING			;load max string size into R1
			BL		NEWLINE            		;print a new line
			MOVS	R0,#0x3E				;load ">" into R0
			BL		PutChar					;print the ">"
			LDR 	R0,=STRINGIN			;load input string address into R0
			BL		GetStringSB				;store input string
			MOVS	R0,#0x3C				;load "<" into R0
			BL		PutChar					;print the "<"
			LDR 	R0,=STRINGIN			;load input string address into R0
			BL		PutStringSB				;print the string
			MOVS	R0,#0x3C				;load "<" into R0
			BL		PutChar					;print the "<"
			BL		NEWLINE            		;print new line
			LDR     R0,=Length				;load address of string "Length:" into R0
			BL      PutStringSB				;print the "Length:"
			LDR 	R0,=STRINGIN			;load input string address into R0
			BL		LengthStringSB			;find the length of the string
			BL		PutNumU					;prints the string length
			BL		NEWLINE            		;print new line
			LDR 	R0,=STRINGIN			;load input string address into R0
			MOVS	R2,#0					;clear R2
			STRB	R2,[R0,#0]				;clear bit 0 of R0
			B		NEWSTRING				;print new line
