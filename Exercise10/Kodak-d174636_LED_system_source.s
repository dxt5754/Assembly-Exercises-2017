            TTL Timer Driver Input Timing
;****************************************************************
;Uses Periodic Interrupt Timer with channel zero
;to implement timing measurements accurate to within 10ms
;Name:  Dean Trivisani
;Date:  11/12/2017
;Class:  CMPE-250
;Section:  01L5
...
MainLoop
			BL		NEWLINE
			;initialize RunStopWatch and Count at 0
			LDR 	R0, =RunStopWatch
			MOVS	R1, #0
			STRB	R1, [R0, #0]
			LDR 	R0, =origCount
			...
			STR 	R1, [R0, #0]
			BL 		Init_PIT_IRQ
			;print the prompt corresponding
			;to the counter
			CMP 	R2, #1
			BEQ		RUN2
			BHI		RUN3
...
PRINTIT		BL 		PutStringSB
			...
			MOVS 	R0, #">"
			BL 		PutChar
			;itnitalize Count to 0
			LDR 	R0, =origCount
			MOVS	R1, #0
			STR 	R1, [R0, #0]
			;initalize RunStopwatch to 1
			LDR 	R0, =RunStopWatch
			MOVS	R1, #1
			STRB	R1, [R0, #0]
			...
			ADDS	R2,R2,#1
			CMP		R2, #3
			;thank user and exit if all
			;prompts have been filled in
			;else loop with next prompt
			BLO		MainLoop
			BL		NEWLINE
			LDR 	R0, =ThxPrompt
			BL		PutStringSB
...
;Interrupt condition is then cleard
			...
			B 		QUITPITISR
PITISRINCR
			LDR 	R0, =origCount					;increment count if stopwatch is running	
			LDR 	R1, [R0, #0]
			ADDS 	R1, R1, #1
			STR 	R1, [R0, #0]
QUITPITISR
...
ThxPrompt	DCB		"Thank you. Goodbye!", 0
TimeFrmtTxt	DCB 	" x 0.01 s", 0
